...
describe('calculatePensionProjection', () => {
  ...
  describe('Edge Cases and Boundary Conditions', () => {
    ...
    test('should handle negative growth rate', () => {
      const result = calculatePensionProjection(30, 100000, 5000, -2, 65, 30000);
    ...
    test('should handle current age equal to maxAge', () => {
      const result = calculatePensionProjection(80, 100000, 5000, 5, 65, 30000, 80);
    ...
    test('should handle current age greater than maxAge', () => {
      const result = calculatePensionProjection(85, 100000, 5000, 5, 65, 30000, 80);
  ...
  describe('Mathematical Precision and Rounding', () => {
    ...
    test('should handle fractional interest rates correctly', () => {
      const result = calculatePensionProjection(30, 10000, 1000, 3.75, 65, 20000);
  ...
  describe('Realistic Retirement Scenarios', () => {
    ...
    test('should handle mid-career catch-up scenario', () => {
      ...
      const result = calculatePensionProjection(45, 200000, 25000, 6, 65, 50000);
    ...
    test('should handle 4% rule scenario', () => {
      ...
      const result = calculatePensionProjection(65, initialPot, 0, 5, 65, annualDrawdown, 95);
  ...
  describe('Stress Tests and Performance', () => {
    test('should handle very long projection period', () => {
      const result = calculatePensionProjection(18, 1000, 500, 5, 67, 10000, 100);
    ...
    test('should handle extreme growth rates', () => {
      ...
      const highGrowthResult = calculatePensionProjection(60, 100000, 5000, 20, 65, 50000);
      const retirementEntry = highGrowthResult.find(entry => entry.age === 65);
      expect(retirementEntry?.potValue).toBeGreaterThan(200000);

      // Very low growth rate  
      const lowGrowthResult = calculatePensionProjection(60, 100000, 5000, 0.5, 65, 50000);
      const retirementEntryLow = lowGrowthResult.find(entry => entry.age === 65);
  ...
  describe('Data Integrity and Consistency', () => {
    ...
    test('should have consistent phase transitions', () => {
      ...
      for (let i = 1; i < result.length; i++) {
        if (result[i-1].phase === 'Accumulation' && result[i].phase === 'Drawdown') {
        ...
        expect(result[i-1].phase === 'Drawdown' && result[i].phase === 'Accumulation').toBe(false);
      }
    ...
    test('should maintain mathematical consistency across years', () => {
      ...
      for (let i = 1; i < result.length; i++) {
        ...
        expect(Math.abs(curr.potValue - Math.round(expectedValue))).toBeLessThanOrEqual(1);
      }
    });
  });
});